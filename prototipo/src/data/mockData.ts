import type { AppState, Appointment, Patient, WaitlistEntry } from '../types'

const patientSeeds: Array<[string, string, number, string, string]> = [
...
export const mockPatients: Patient[] = patientSeeds.map(([id, name, age, neighbourhood, gender], index) => ({
  id, name, age, neighbourhood, gender,
  comorbidities: index % 3 === 0 ? ['Hipertensão (dado fictício)'] : [],
  preferences: {
    channels: index % 2 ? ['E-mail', 'Push'] : ['WhatsApp', 'Push'],
    phone: `(11) 90000-${String(1000 + index)}`,
    email: `${name.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(' ', '.')}@exemplo.test`,
  },
}))

const appointmentSeeds: Array<[string, string, string, string, string, string, Appointment['status'], number, number, boolean]> = [
...
const level = (probability: number): Appointment['riskLevel'] => probability >= .65 ? 'HIGH' : probability >= .4 ? 'MEDIUM' : 'LOW'

export const mockAppointments: Appointment[] = appointmentSeeds.map(([id, patientId, specialty, unit, date, time, status, riskProbability, waitingDays, smsReceived]) => ({
  id, patientId, specialty, unit, date, time, status, riskProbability, riskLevel: level(riskProbability), waitingDays, smsReceived,
...
const waitlistSeeds: Array<[string, string, string, string, ('morning' | 'afternoon')[], number, string, WaitlistEntry['preferredContact']]> = [
...
export const mockWaitlist: WaitlistEntry[] = waitlistSeeds.map(([id, patientId, specialty, unit, availablePeriods, queuePriority, enteredAt, preferredContact]) => ({
  id, patientId, specialty, unit, availablePeriods, queuePriority, enteredAt, preferredContact, status: 'AGUARDANDO',
}))
